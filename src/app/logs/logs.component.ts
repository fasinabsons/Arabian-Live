...
fetchDataAbsons(): void {
  ...
  collections.forEach((collection) => {
    this.http.get<{ data: any[] }>(`http://localhost:3000/fetch-collection/${collection}`).subscribe(
      (response) => {
...
fetchFromBackendAbsons(collection: string): void {
  this.isRefreshing = true;
  this.http.get<any[]>(`http://localhost:3000/fetch-collection/${collection}`).subscribe(
  ...
  this.http.get<any[]>(`http://localhost:3000/get-local-data/deliveryorders`).subscribe(
