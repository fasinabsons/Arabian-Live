...
export class ForecastComponent implements OnInit {
  ...
  fetchOrders(): void {
    this.http.get('http://localhost:3000/fetch-collection/salesorders').subscribe(
      (response: any) => {
  ...
  fetchPurchaseOrders(): void {
    this.http.get('http://localhost:3000/fetch-collection/purchaseorders').subscribe(
      (response: any) => {
  ...
  fetchEstimations(): void {
    this.http.get('http://localhost:3000/fetch-collection/estimations').subscribe(
      (response: any) => {
  ...
  calculateGrowth(values: number[]): number {
   ...
   renderLineChart(): void {
    ...
    const salesData = this.aggregateData(this.salesorders, 'salesOrderDate');
    const purchaseData = this.aggregateData(this.purchaseorders, 'PODate');
