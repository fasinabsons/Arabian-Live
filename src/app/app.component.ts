...
private socket!: WebSocket;
  ...
  // ✅ Fetch Saved Logs
  fetchSavedLogs(): void {
    this.http.get<any[]>('http://localhost:3000/get-system-logs')
      .subscribe(logs => this.systemLogs = logs || []);
  }

  // ✅ Download Logs
  downloadLogs(): void {
    window.open('http://localhost:3000/download-logs/csv', '_blank');
  }
  ...
  fetchData(): void {
    ...
    this.http.get<any[]>(`http://localhost:3000/get-local-data/${this.collectionName}`).subscribe(
      (data) => {
  ...
  private fetchDataFromBackend(): void {
    this.http.get<any[]>(`http://localhost:3000/fetch-collection/${this.collectionName}`).subscribe(
      (response) => {
...
loadFilterSelections(): void {
  ...
  this.http.get<{ selectedFields: string[]; nestedSelectedFields: { [key: string]: string[] } }>(
    `http://localhost:3000/load-filter-selections/${this.collectionName}`
  ).subscribe(
    (response) => {
...
saveFilterSelections(): void {
  ...
  // Save to backend
  this.http.post(`http://localhost:3000/save-filter-selections/${this.collectionName}`, filterData).subscribe(
...
saveFilteredData(): void {
  ...
  this.http.post(`http://localhost:3000/save-filter-data/${this.collectionName}`, documentPayload).subscribe(
...
fetchFilteredData(): void {
  this.http.get<{ filteredData: any[] }>(`http://localhost:3000/get-filtered-data/${this.collectionName}`).subscribe(
    (data) => {
